...
care::care(CACHE* cache)
    : replacement(cache), num_sets(cache->NUM_SET), num_ways(cache->NUM_WAY), blocks(static_cast<std::size_t>(cache->NUM_SET * cache->NUM_WAY)),
      sampled_sets(static_cast<std::size_t>(cache->NUM_SET), false), sht(NUM_CPUS)
{
...
std::size_t care::block_index(long set, long way) const { return static_cast<std::size_t>(set) * static_cast<std::size_t>(num_ways) + static_cast<std::size_t>(way); }
...
std::size_t care::signature_index(uint16_t signature) const { return static_cast<std::size_t>(signature) & (sht_entries - 1); }
...
long care::find_victim(uint32_t, uint64_t, long set, const champsim::cache_block* current_set, champsim::address, champsim::address, access_type)
{
...
void care::update_replacement_state(uint32_t triggering_cpu, long set, long way, champsim::address, champsim::address, champsim::address, access_type type,
                                    uint8_t hit)
{
  if (!hit)
    return;
...
void care::replacement_cache_fill(uint32_t triggering_cpu, long set, long way, champsim::address, champsim::address ip, champsim::address, access_type type,
                                  double pmc)
{
  auto idx = block_index(set, way);
